...
```rs
fn call(agent: &mut Agent, mut gc: Gc<'_>, scope: Scope<'_>, value: Value) -> Value {
    // ...
}
```
...
```rs
fn call(agent: &mut Agent, mut gc: Gc<'_>, scope: Scope<'_>, value: Register<Value<'_>>) -> Register<Value<'_>> {
...
```rs
fn call(agent: &mut Agent, mut gc: Gc<'_>, scope: Scope<'_>, value: Register<Value<'_>>) -> JsResult<Register<Value<'_>>> {
    let value = unsafe { value.bind(agent) };
    let kept_value: Global<Value<'static>> = value.make_global(value);
    other_call(agent, gc, scope,value.into_register())?;
