...
use crate::{
    ...
    engine::{
        ActiveIterator, Bindable, GcScope, NoGcScope, Scopable, ScopableCollection,
        bytecode::{
            FunctionExpression, Instruction, NamedEvaluationParameter,
...
use super::{
    ExceptionHandler, apply_string_or_numeric_addition, apply_string_or_numeric_binary_operator,
    bigint_binary_operator,
...
pub(super) fn execute_await_promise_resolve<'gc>(
    ...
    let value = agent.vm.take_result(gc.nogc());
    // 1. Let asyncContext be the running execution context.
    // 2. Let promise be ? PromiseResolve(%Promise%, value).
    let promise = Promise::resolve(agent, value.unbind(), gc)?;
...
pub(super) fn execute_array_push<'gc>(
    ...
    let value = agent.vm.take_result(gc.nogc());
    let array = agent.vm.get_last_stack_value(gc.nogc());
...
pub(super) fn execute_bitwise_not<'gc>(
    ...
    let old_value = Numeric::try_from(agent.vm.take_result(gc))
        .unwrap()
        .bind(gc);
...
pub(super) fn execute_unary_minus(agent: &mut Agent, gc: NoGcScope) {
    let old_value = agent.vm.take_result(gc);
...
pub(super) fn execute_to_number<'gc>(agent: &mut Agent, gc: GcScope<'gc, '_>) -> JsResult<'gc, ()> {
    let arg0 = agent.vm.take_result(gc.nogc());
...
pub(super) fn execute_to_object<'gc>(
    ...
    let value = agent.vm.take_result(gc);
    agent.vm.result = Some(to_object(agent, value, gc)?.unbind().into());
    Ok(())
}
...
pub(super) fn execute_apply_addition_binary_operator<'gc>(
    ...
    let lval = agent.vm.pop_stack(gc.nogc());
    let rval = agent.vm.take_result(gc.nogc());
    let result = if let (Ok(lnum), Ok(rnum)) = (Number::try_from(lval), Number::try_from(rval)) {
        ...
        String::concat(agent, [lstr.unbind(), rstr.unbind()], gc.into_nogc()).into()
    } else if let (Ok(lnum), Ok(rnum)) = (BigInt::try_from(lval), BigInt::try_from(rval)) {
        BigInt::add(agent, lnum, rnum).into()
    } else {
        apply_string_or_numeric_addition(agent, lval.unbind(), rval.unbind(), gc)?
    };
    agent.vm.result = Some(result.unbind());
    Ok(())
}
...
pub(super) fn execute_object_define_property<'gc>(
    ...
    let key = agent.vm.pop_stack(gc.nogc());
    agent.vm.execute_load();
    let key = to_property_key(agent, key.unbind(), gc.reborrow())
        .unbind()?
        .bind(gc.nogc());
    let key = key.unbind().bind(gc.nogc());
    let value = agent.vm.pop_stack(gc.nogc());
    let object = agent.vm.get_last_stack_value(gc.nogc());
...
pub(super) fn execute_object_define_method<'gc>(
    ...
    let prop_key = agent.vm.pop_stack(gc.nogc());
    let prop_key = to_property_key(agent, prop_key.unbind(), gc.reborrow())
        .unbind()?
        .bind(gc.nogc());
    let object = Object::try_from(agent.vm.get_last_stack_value(gc.nogc()))
        .unwrap()
        .bind(gc.nogc());
...
pub(super) fn execute_object_set_prototype<'gc>(
    ...
    let prop_value = agent.vm.take_result(gc.nogc());
...
pub(super) fn execute_get_value<'gc>(
    ...
    let result = match result {
        ...
        _ => {
            if keep_reference {
                agent.vm.execute_push_reference();
            }
            let result = handle_get_value_break(agent, &reference.unbind(), result.unbind(), gc);
            if keep_reference {
                agent.vm.execute_pop_reference();
            }
            result?
        }
    };
...
pub(super) fn execute_copy_data_properties<'gc>(
    ...
    let source = agent.vm.take_result(gc.nogc());
    let Value::Object(target) = agent.vm.get_last_stack_value(gc.nogc()) else {
        unreachable!()
    };
    copy_data_properties(agent, target.unbind(), source.unbind(), gc)?;
    Ok(())
}
...
pub(super) fn execute_copy_data_properties_into_object<'gc>(
    ...
    let from = Object::try_from(agent.vm.take_result(gc.nogc())).unwrap();
    ...
    for _ in 0..num_excluded_items {
        let reference = agent.vm.pop_reference_stack();
        ...
        try_copy_data_properties_into_object(agent, from, &excluded_items, gc.nogc())
    {
        ...
        let result = copy_data_properties_into_object(agent, from, excluded_items, gc)?;
        agent.vm.result = Some(result.unbind().into());
    }
    Ok(())
}
    ...
    let name = if let Some(parameter) = &identifier {
        let pk_result = match parameter {
            ...
            NamedEvaluationParameter::Stack => {
                let value = agent.vm.get_last_stack_value(gc.nogc());
                if let Some(pk) = to_property_key_simple(agent, value, gc.nogc()) {
                    Ok(pk)
                } else {
                    Err(value)
        ...
        match pk_result {
            ...
            Err(pk_value) => {
                ...
                let pk = to_property_key_complex(agent, pk_value, gc.reborrow())
                    .unbind()?
                    .bind(gc.nogc());
                // SAFETY: not shared.
...
pub(super) fn execute_instantiate_ordinary_function_expression<'gc>(
    ...
    let (name, env, initialize_binding) = if let Some(parameter) = identifier {
        ...
        let pk = match parameter {
            NamedEvaluationParameter::Result => agent.vm.take_result(gc.nogc()),
            NamedEvaluationParameter::Stack => agent.vm.get_last_stack_value(gc.nogc()),
        };
        let name = to_property_key(agent, pk.unbind(), gc.reborrow())
            .unbind()?
            .bind(gc.nogc());
...
pub(super) fn execute_class_define_constructor<'gc>(
    ...
    let name = agent.vm.pop_stack(gc.nogc());
    let class_name = set_class_name(agent, name.unbind(), gc.reborrow())
        .unbind()?
        .bind(gc.nogc());
    ...
    let function_prototype = if has_constructor_parent {
        Some(Object::try_from(agent.vm.pop_stack(gc.nogc())).unwrap())
    } else {
        None
    };
    let proto = OrdinaryObject::try_from(agent.vm.get_last_stack_value(gc.nogc())).unwrap();
...
pub(super) fn execute_class_define_default_constructor<'gc>(
    ...
    let (compiled_initializer_bytecode, has_constructor_parent) = agent
        .current_executable(gc.nogc())
    ...
    let proto = Object::try_from(agent.vm.get_last_stack_value(gc.nogc())).unwrap();
...
pub(super) fn execute_class_define_private_method<'gc>(
    ...
    let description = String::try_from(agent.vm.take_result(gc.nogc())).unwrap();
    let FunctionExpression { expression, .. } = agent
        .current_executable(gc.nogc())
        .fetch_function_expression(agent, instr.get_first_index(), gc.nogc());
...
pub(super) fn execute_class_define_private_property<'gc>(
    ...
    if is_static {
        ...
        let object = agent.vm.get_last_stack_value(gc);
        let object = Object::try_from(object).unwrap();
...
pub(super) fn execute_class_initialize_private_elements<'gc>(
    ...
    let target = Object::try_from(agent.vm.get_last_stack_value(gc)).unwrap();
    target
        .get_or_create_backing_object(agent)
        .property_storage()
...
pub(super) fn execute_direct_eval_call<'gc>(
    ...
    let func_ref = resolve_binding(agent, BUILTIN_STRING_MEMORY.eval, None, None, gc.reborrow())
        .unbind()?
        .bind(gc.nogc());
    let func = get_value(agent, &func_ref.unbind(), gc.reborrow())
        .unbind()?
        .bind(gc.nogc());
    let args = agent.vm.get_call_args(instr, gc.nogc());
    ...
    let result = if func == agent.current_realm_record().intrinsics().eval().into() {
        ...
        if args.is_empty() {
            ...
            perform_eval(agent, eval_arg, true, strict_caller, gc)?
        }
    } else {
...
pub(super) fn execute_evaluate_call<'gc>(
    ...
    let mut args = agent.vm.get_call_args(instr, gc.nogc()).unbind();
    let func = agent.vm.pop_stack(gc.nogc()).unbind();
...
pub(super) fn execute_evaluate_new<'gc>(
    ...
    let args = agent.vm.get_call_args(instr, gc.nogc());
    let constructor = agent.vm.pop_stack(gc.nogc()).bind(gc.nogc());
    let Some(constructor) = is_constructor(agent, constructor) else {
        let constructor_string = constructor.unbind().string_repr(agent, gc.reborrow());
...
pub(super) fn execute_evaluate_super<'gc>(
    ...
    let arg_list = agent.vm.get_call_args(instr, gc.nogc());
    // 5. If IsConstructor(func) is false, throw a TypeError exception.
    let Some(func) = func.and_then(|func| is_constructor(agent, func)) else {
        let constructor = func.map_or(Value::Null, |f| f.into());
        let error_message = format!(
            "'{}' is not a constructor.",
            constructor
                .unbind()
...
pub(super) fn execute_evaluate_property_access_with_expression_key(
    ...
    let property_name_value = agent.vm.take_result(gc);
    let base_value = agent.vm.pop_stack(gc);
...
pub(super) fn execute_evaluate_property_access_with_identifier_key(
    ...
    let base_value = agent.vm.take_result(gc);
    let strict = agent.is_evaluating_strict_code();
...
#[inline(always)]
pub(super) fn execute_jump(agent: &mut Agent, instr: Instr) {
...
#[inline(always)]
pub(super) fn execute_jump_if_not(agent: &mut Agent, instr: Instr, gc: NoGcScope) {
    let result = agent.vm.take_result(gc);
    let ip = instr.get_jump_slot();
...
#[inline(always)]
pub(super) fn execute_jump_if_true(agent: &mut Agent, instr: Instr, gc: NoGcScope) {
    let result = agent.vm.take_result(gc);
...
pub(super) fn execute_increment(agent: &mut Agent, gc: NoGcScope) {
    let lhs = agent.vm.take_result(gc);
...
pub(super) fn execute_less_than<'gc>(agent: &mut Agent, gc: GcScope<'gc, '_>) -> JsResult<'gc, ()> {
    let lval = agent.vm.pop_stack(gc.nogc());
    let rval = agent.vm.take_result(gc.nogc());
    let result = is_less_than::<true>(agent, lval.unbind(), rval.unbind(), gc)?;
...
pub(super) fn execute_less_than_equals<'gc>(
    ...
    let lval = agent.vm.pop_stack(gc.nogc());
    let rval = agent.vm.take_result(gc.nogc());
    let result = is_less_than::<false>(agent, rval.unbind(), lval.unbind(), gc)?;
    ...
    let result = match try_has_property(agent, rval, property_key, None, gc.nogc()) {
        ControlFlow::Continue(c) => match c {
            ...
            TryHasResult::Proxy(proxy) => {
                ...
                proxy.internal_has_property(agent, property_key, gc)?
            }
        },
        ControlFlow::Break(TryError::Err(err)) => {
            return Err(err.unbind().bind(gc.into_nogc()));
...
pub(super) fn execute_is_strictly_equal(agent: &mut Agent, gc: NoGcScope) {
    let lval = agent.vm.pop_stack(gc);
    let rval = agent.vm.take_result(gc);
...
pub(super) fn execute_is_loosely_equal<'gc>(
    ...
    let lval = agent.vm.pop_stack(gc.nogc());
    let rval = agent.vm.take_result(gc.nogc());
    let result = is_loosely_equal(agent, lval.unbind(), rval.unbind(), gc)?;
    agent.vm.result = Some(result.into());
    Ok(())
}

#[inline(always)]
pub(super) fn execute_is_constructor(agent: &mut Agent, gc: NoGcScope) {
    let val = agent.vm.take_result(gc);
...
#[inline(always)]
pub(super) fn execute_logical_not(agent: &mut Agent, gc: NoGcScope) {
    let value = agent.vm.take_result(gc);
    // 2. Let oldValue be ToBoolean(? GetValue(expr)).
    let old_value = to_boolean(agent, value);
...
pub(super) fn execute_initialize_variable_environment<'gc>(
    ...
    for _ in 0..num_variables {
        let n = String::try_from(agent.vm.pop_stack(gc)).unwrap();
        let initial_value = agent.vm.pop_stack(gc);
...
pub(super) fn execute_enter_class_static_element_environment(agent: &mut Agent, gc: NoGcScope) {
    let class_constructor = Function::try_from(agent.vm.get_last_stack_value(gc)).unwrap();
...
pub(super) fn execute_throw_error<'gc>(
    ...
    let message = String::try_from(agent.vm.take_result(gc)).unwrap();
...
pub(super) fn execute_push_exception_jump_target(agent: &mut Agent, instr: Instr, gc: NoGcScope) {
    agent
        .vm
...
pub(super) fn execute_instanceof_operator<'gc>(
    ...
    let lval = agent.vm.pop_stack(gc.nogc());
    let rval = agent.vm.take_result(gc.nogc());
    let result = instanceof_operator(agent, lval.unbind(), rval.unbind(), gc)?;
    agent.vm.result = Some(result.into());
    Ok(())
}
...
pub(super) fn execute_begin_simple_array_binding_pattern<'gc>(
    ...
    execute_simple_array_binding(agent, env, gc).unbind()?;
    Ok(())
}
...
pub(super) fn execute_begin_simple_object_binding_pattern<'gc>(
    ...
    let result = agent.vm.take_result(gc.nogc());
    let object = to_object(agent, result, gc.nogc())
        .unbind()?
        .bind(gc.nogc());
    execute_simple_object_binding(agent, object.unbind(), env, gc)
}
...
pub(super) fn execute_string_concat<'gc>(
    ...
    let string = if all_easy {
        let gc = gc.nogc();
        let args = NonNull::from_mut(&mut agent.vm.stack[first_arg_index..]);
        let len = args.len();
        let start = args.cast::<Primitive>();
        for i in 0..len {
            // SAFETY: args are in start..end, and calling to_string_primitive
            // cannot call into the VM and therefore the pointer range is
            // untouched.
            let arg = unsafe { start.add(i) };
            // SAFETY: reading inside the Agent while we have exclusive access.
            let value = unsafe { arg.read() }.bind(gc);
            let string: String<'_> = to_string_primitive(agent, value, gc).unwrap();
            length += string.len_(agent);
            // SAFETY: Writing back out into arg while we have exclusive access.
            unsafe { arg.write(string.unbind().into()) };
        }
        // SAFETY: String is a sub-enum of Value and we've written a String into
        // each of the args.
        let args = unsafe { std::mem::transmute::<NonNull<[Value<'_>]>, &[String<'_>]>(args) };
...
pub(super) fn execute_enumerate_object_properties(agent: &mut Agent, gc: NoGcScope) {
    let result = agent.vm.take_result(gc);
    let object = to_object(agent, result, gc).unwrap();
    agent.vm.iterator_stack.push(
...
pub(super) fn execute_get_iterator_sync<'gc>(
    ...
    let expr_value = agent.vm.take_result(gc.nogc());
    let result = VmIteratorRecord::from_value(agent, expr_value.unbind(), gc)?;
    agent.vm.iterator_stack.push(result.unbind());
    Ok(())
}
...
pub(super) fn execute_get_iterator_async<'gc>(
    ...
    let expr_value = agent.vm.take_result(gc.nogc());
    let result = VmIteratorRecord::async_from_value(agent, expr_value.unbind(), gc)?;
    agent.vm.iterator_stack.push(result.unbind());
    Ok(())
}
...
pub(super) fn execute_iterator_step_value<'gc>(
    ...
    let result = ActiveIterator::new(agent, gc.nogc()).step_value(agent, gc)?;
...
pub(super) fn execute_iterator_step_value_or_undefined<'gc>(
    ...
    let result = if let Some(r) = try_result_into_option_js(
        // SAFETY: try_step_value does not access the VM.
        unsafe { NonNull::from_mut(agent.vm.get_active_iterator_mut()).as_mut() }
    ...
    if result.map_or(true, |r| r.is_none()) {
        ...
        *agent.vm.get_active_iterator_mut() = VmIteratorRecord::EmptySliceIterator;
    }
...
pub(super) fn execute_iterator_call_next_method<'gc>(
    ...
    let result = agent.vm.take_any_result(gc.nogc());
    agent.vm.result = Some(
        ActiveIterator::new(agent, gc.nogc())
            .call_next(agent, result.unbind(), gc)?
            .unbind(),
    );
    Ok(())
}
...
pub(super) fn execute_iterator_complete<'gc>(
    ...
    if done {
        ...
        let result = iterator_value(agent, result.unbind(), gc)?;
        agent.vm.result = Some(result.unbind());
        let ip = instr.get_jump_slot();
        if agent.options.print_internals {
...
pub(super) fn execute_iterator_throw<'gc>(
    ...
    let result = agent.vm.take_result(gc.nogc());
    let result = ActiveIterator::new(agent, gc.nogc()).throw(agent, result.unbind(), gc)?;
...
pub(super) fn execute_iterator_return<'gc>(
    ...
    let value = ActiveIterator::new(agent, gc.nogc()).r#return(agent, Some(result), gc)?;
...
pub(super) fn execute_iterator_rest_into_array<'gc>(
    ...
    while let Some(value) =
        match ActiveIterator::new(agent, gc.nogc()).step_value(agent, gc.reborrow()) {
            Ok(value) => value,
            Err(err) => {
                // Iterator threw an error; replace it with an empty slice
                // iterator so further instructions aren't observable.
                *agent.vm.get_active_iterator_mut() = VmIteratorRecord::EmptySliceIterator;
    ...
    // We exhausted the iterator; replace it with an empty slice iterator so
    // further instructions aren't observable.
    *agent.vm.get_active_iterator_mut() = VmIteratorRecord::EmptySliceIterator;
...
pub(super) fn execute_iterator_close<'gc>(
    ...
    let load_result_to_stack = agent.vm.result.is_some();
    if load_result_to_stack {
        agent.vm.execute_load();
    }
    let result = ActiveIterator::new(agent, gc.nogc())
        .r#return(agent, None, gc.reborrow())
        .unbind()?
        .bind(gc.nogc());
    if load_result_to_stack {
        agent.vm.execute_store();
...
pub(super) fn execute_async_iterator_close<'gc>(
    ...
    let result = ActiveIterator::new(agent, gc.nogc()).r#return(agent, None, gc)?;
    if load_result_to_stack {
        agent.vm.execute_store();
    }
    if let Some(result) = result {
...
pub(super) fn execute_iterator_close_with_error(agent: &mut Agent, gc: GcScope) {
        ...
        .requires_return_call(agent, gc.nogc())
    {
        ...
        // let load_result_to_stack = agent.vm.result.is_some();
        debug_assert!(agent.vm.result.is_some());
        agent.vm.execute_load();
        let _ = ActiveIterator::new(agent, gc.nogc()).r#return(agent, None, gc);
        agent.vm.execute_store();
    }
...
pub(super) fn execute_async_iterator_close_with_error(agent: &mut Agent, gc: GcScope) -> bool {
    ...
    if unsafe { NonNull::from_ref(agent.vm.get_active_iterator()).as_ref() }
        .requires_return_call(agent, gc.nogc())
    {
        debug_assert!(agent.vm.result.is_none());
        let inner_result_value = ActiveIterator::new(agent, gc.nogc()).r#return(agent, None, gc);
...
pub(super) fn execute_create_unmapped_arguments_object<'gc>(
    ...
    let Some(VmIteratorRecord::SliceIterator(slice)) = agent.vm.iterator_stack.last() else {
        unreachable!()
    };
...
pub(super) fn execute_import_call(agent: &mut Agent, gc: GcScope) {
    let specifier = agent.vm.pop_stack(gc.nogc());
    let options = agent.vm.take_any_result(gc.nogc());
    agent.vm.result = Some(
        evaluate_import_call(agent, specifier.unbind(), options.unbind(), gc)
            .unbind()
            .into(),
    );
}
    ...
    if is_unresolvable_reference(&reference) {
        ...
        let referenced_name = if !reference.is_static_property_reference() {
            ...
            if let Some(referenced_name) = to_property_key_simple(agent, referenced_name, gc.nogc())
            {
                ...
                let referenced_name =
                    to_property_key_complex(agent, referenced_name.unbind(), gc.reborrow())
                        .unbind()?
                        .bind(gc.nogc());
                // SAFETY: not shared.
            ...
            base_obj.try_delete(agent, referenced_name, gc.nogc())
        {
            ...
            base_obj
                .unbind()
                .internal_delete(agent, referenced_name.unbind(), gc.reborrow())
                .unbind()?
                .bind(gc.nogc())
        };
                ...
                .unbind()?
        {
            ...
            base.unbind()
                .delete_binding(agent, referenced_name.unbind(), gc)?
        };
        agent.vm.result = Some(result.into());
    }
    Ok(())
...
fn handle_set_value_break<'gc>(
    ...
    match result {
        ...
        ControlFlow::Break(TryError::GcError) => put_value(agent, reference, value, gc),
    }
}
