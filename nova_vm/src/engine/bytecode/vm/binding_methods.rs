...
use crate::{
    ...
    engine::{
        ActiveIterator, Bindable, GcScope, Instr, Instruction, Scopable, ScopableCollection,
        Scoped, VmIteratorRecord,
    },
};
...
pub(super) fn execute_simple_array_binding<'a>(
    ...
    let instructions = agent.current_executable(gc.nogc()).get_instructions(agent);
    loop {
        let instr = Instr::consume_instruction(instructions, &mut agent.vm.ip).unwrap();
        ...
        match instr.kind {
            ...
            Instruction::BindingPatternBind | Instruction::BindingPatternBindRest => {
                ...
                let binding_id = agent.current_executable(gc.nogc()).fetch_identifier(
                    agent,
    ...
    loop {
        ...
        match instr.kind {
            ...
            Instruction::BindingPatternBind | Instruction::BindingPatternBindNamed => {
                let exe = agent.current_executable(gc.nogc());
                let binding_id = exe.fetch_identifier(agent, instr.get_first_index(), gc.nogc());

                let property_key = if instr.kind == Instruction::BindingPatternBind {
                    binding_id.into()
                } else {
                    let key_value = exe.fetch_constant(agent, instr.get_second_index(), gc.nogc());
            ...
            Instruction::BindingPatternBindToIndex => {
                let key_value = agent.current_executable(gc.nogc()).fetch_constant(
                    agent,
            ...
            Instruction::BindingPatternGetValueNamed => {
                ...
                let property_key = unsafe {
                    PropertyKey::from_value_unchecked(
                        agent.current_executable(gc.nogc()).fetch_constant(
                            agent,
                            instr.get_first_index(),
                            gc.nogc(),
                        ),
            ...
            Instruction::BindingPatternBindRest => {
                ...
                if environment.is_none() {
                    put_value(agent, &lhs, rest_obj.unbind().into(), gc.reborrow()).unbind()?;
                } else {
                    initialize_referenced_binding(
                        agent,
                        lhs,
...
pub(super) fn execute_nested_simple_binding<'a>(
    ...
    let instr = Instr::consume_instruction(
        agent.current_executable(gc.nogc()).get_instructions(agent),
        &mut agent.vm.ip,
    )
    .unwrap();
    if agent.options.print_internals {
    ...
    match instr.kind {
        Instruction::BeginSimpleArrayBindingPattern => {
            let result = VmIteratorRecord::from_value(agent, value.unbind(), gc.reborrow())
                .unbind()?
                .bind(gc.nogc());
            agent.vm.iterator_stack.push(result.unbind());
            let result = execute_simple_array_binding(agent, environment, gc.reborrow()).unbind();
            let gc = gc.into_nogc();
            let result = result.bind(gc);
            agent.vm.pop_iterator(gc);
            result
        }
        Instruction::BeginSimpleObjectBindingPattern => {
            let object = to_object(agent, value.unbind(), gc.nogc())
                .unbind()?
                .bind(gc.nogc());
            execute_simple_object_binding(agent, object.unbind(), environment, gc)
        }
        _ => unreachable!(),
    }
}
