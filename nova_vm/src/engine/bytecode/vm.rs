...
use crate::{
    ecmascript::{
        ...
        execution::{
            Agent, Environment, JsResult, PrivateField, PrivateMethod, ProtoIntrinsics,
    ...
    fn execute_instruction<'a>(
        ...
        match instr.kind {
            ...
            Instruction::ClassInitializePrivateValue => {
                ...
                println!(
                    "Setting private name {private_name:?} with description {}",
                    private_env
                        .resolve_description(agent, private_name, gc)
                        .unwrap()
                        .as_str(agent)
                );
                unwrap_try(this.try_set(agent, private_name.into(), value, this.into_value(), gc));
            }
            Instruction::Swap => {
