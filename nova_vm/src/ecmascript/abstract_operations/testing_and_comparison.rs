...
pub(crate) fn is_extensible(
    ...
    o.internal_is_extensible(agent, gc, scope)
}
...
pub(crate) fn is_less_than<const LEFT_FIRST: bool>(
    ...
    // 4. Else,
        ...
        let nx = to_numeric(agent, gc.reborrow(), scope.reborrow(), px)?;

        // e. Let ny be ? ToNumeric(py).
        let ny = to_numeric(agent, gc.reborrow(), scope.reborrow(), py)?;
        ...
        let rnx = nx.to_real(agent, gc.reborrow(), scope.reborrow())?;
        let rny = nx.to_real(agent, gc, scope)?;
        Ok(Some(rnx < rny))
    }
}
...
pub(crate) fn is_loosely_equal(
    ...
    if let (Ok(x), Ok(y)) = (Number::try_from(x), String::try_from(y)) {
        ...
        let y = to_number(agent, gc.reborrow(), scope.reborrow(), y).unwrap();
        return Ok(is_loosely_equal(agent, gc, scope, x, y).unwrap());
    }

    // 6. If x is a String and y is a Number, return ! IsLooselyEqual(! ToNumber(x), y).
    if let (Ok(x), Ok(y)) = (String::try_from(x), Number::try_from(y)) {
        // TODO: We know GC cannot be triggered here.
        let x = to_number(agent, gc.reborrow(), scope.reborrow(), x).unwrap();
        return Ok(is_loosely_equal(agent, gc, scope, x, y).unwrap());
    }
    ...
    if let (Ok(x), Ok(y)) = (BigInt::try_from(x), String::try_from(y)) {
        ...
        if let Some(n) = string_to_big_int(agent, y) {
            ...
            // TODO: We know GC cannot be triggered here.
            return Ok(is_loosely_equal(agent, gc, scope, x, n).unwrap());
        } else {
            return Ok(false);
        }
    }
    ...
    if let (Ok(x), Ok(y)) = (String::try_from(x), BigInt::try_from(y)) {
        // TODO: We know GC cannot be triggered here.
        return Ok(is_loosely_equal(agent, gc, scope, y, x).unwrap());
    }
    ...
    if (x.is_string() || x.is_number() || x.is_bigint() || x.is_symbol()) && y.is_object() {
        let y = to_primitive(agent, gc.reborrow(), scope.reborrow(), y, None)?;
        return Ok(is_loosely_equal(agent, gc, scope, x, y).unwrap());
    }

    // 12. If x is an Object and y is either a String, a Number, a BigInt, or a Symbol, return ! IsLooselyEqual(? ToPrimitive(x), y).
    if x.is_object() && (y.is_string() || y.is_number() || y.is_bigint() || y.is_symbol()) {
        let x = to_primitive(agent, gc.reborrow(), scope.reborrow(), x, None)?;
        return Ok(is_loosely_equal(agent, gc, scope, x, y).unwrap());
    }

    // 13. If x is a BigInt and y is a Number, or if x is a Number and y is a BigInt, then
    if let Some(xy) = if x.is_bigint() {
        y.to_number(agent, gc.reborrow(), scope.reborrow()).ok()
    } else if y.is_bigint() {
        x.to_number(agent, gc.reborrow(), scope.reborrow()).ok()
    } else {
        None
    } {
        ...
        let rx = x.to_real(agent, gc.reborrow(), scope.reborrow())?;
        let ry = y.to_real(agent, gc, scope)?;
        return Ok(rx == ry);
    }
