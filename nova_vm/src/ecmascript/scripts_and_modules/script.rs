...
use crate::{
    ecmascript::{
        ...
        types::{HeapString, IntoValue, String, Value, BUILTIN_STRING_MEMORY},
    },
...
pub struct Script {
    ...
    pub(crate) source_text: HeapString,

    /// The arena that contains the parsed data of the script source.
...
pub fn parse_script(
    ...
    // Nova custom step: If the script source string is not a heap string,
    ...
    // the script to keep references to the string buffer.
    let (source_text, source_text_buffer) = match source_text {
        String::String(x) => {
            // SAFETY: Script keeps this string alive while functions still refer
        ...
        String::SmallString(x) => {
            // Add 10 whitespace bytes to the end of the script string. This
    ...
    let mut allocator = NonNull::from(Box::leak(Default::default()));
    // SAFETY: Allocator has just been leaked: it's safe to refer to it mutably.
    let parser = Parser::new(
        unsafe { allocator.as_mut() },
        source_text_buffer,
        source_type,
    );
    let ParserReturn {
    ...
    if !errors.is_empty() {
        ...
        drop(program);
        // Parsing failed; the script never runs anything and it's thus safe to
        // drop the allocator.
    ...
    let SemanticBuilderReturn { errors, .. } =
        SemanticBuilder::new(source_text_buffer, source_type)
    ...
    if !errors.is_empty() {
        // Make sure `program` can't borrow `source_text` so we can return it.
        drop(program);
        // Semantic analysis failed; the script never runs anything and it's
        // thus safe to drop the allocator.
        drop(unsafe { Box::from_raw(allocator.as_mut()) });
        return Err(errors);
    }
