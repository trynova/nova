...
pub(crate) fn set_default_global_bindings(
    ...
    // 19.1 Value Properties of the Global Object
    {
        ...
        define_property_or_throw(agent, gc.reborrow(), scope.reborrow(), global, name, desc)?;
...
pub(crate) fn initialize_host_defined_realm(
    ...
    mut gc: Gc<'_>,
    scope: Scope<'_>,
    create_global_object: Option<impl FnOnce(&mut Agent, Gc<'_>, Scope<'_>) -> Object>,
    create_global_this_value: Option<impl FnOnce(&mut Agent, Gc<'_>, Scope<'_>) -> Object>,
    initialize_global_object: Option<impl FnOnce(&mut Agent, Gc<'_>, Scope<'_>, Object)>,
) {
    ...
    let global = create_global_this_value.map(|create_global_this_value| {
        create_global_this_value(agent, gc.reborrow(), scope.reborrow())
    });
    ...
    let this_value = create_global_object
        .map(|create_global_object| create_global_object(agent, gc.reborrow(), scope.reborrow()));
    ...
    let global_object =
        set_default_global_bindings(agent, gc.reborrow(), scope.reborrow(), realm).unwrap();

    // 11. Create any host-defined global object properties on globalObj.
    if let Some(initialize_global_object) = initialize_global_object {
        initialize_global_object(agent, gc.reborrow(), scope.reborrow(), global_object);
    };

    // 12. Return UNUSED.
}

pub(crate) fn initialize_default_realm(agent: &mut Agent, gc: Gc<'_>, scope: Scope<'_>) {
    let create_global_object: Option<fn(&mut Agent, Gc<'_>, Scope<'_>) -> Object> = None;
    let create_global_this_value: Option<fn(&mut Agent, Gc<'_>, Scope<'_>) -> Object> = None;
    let initialize_global_object: Option<fn(&mut Agent, Gc<'_>, Scope<'_>, Object)> = None;
...
mod test {
    ...
    use crate::{
        engine::context::{Gc, GcToken, Scope, ScopeToken},
        heap::{
    ...
    fn test_default_realm_sanity() {
        ...
        initialize_default_realm(&mut agent, gc, scope);
        assert_eq!(
