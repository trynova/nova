...
impl PrivateEnvironmentRecord {
    ...
    /// Get the combined number of instance private fields and methods in this
    /// Private Environment.
    pub(crate) fn get_instance_private_element_count(&self) -> u32 {
        self.instance_private_field_count + self.instance_private_method_count
    }

    /// Get the first PrivateMethods field PrivateName and a list of all the
    /// PrivateMethods in this PrivateEnvironment.
    pub(crate) fn get_instance_private_methods<'gc>(
        &self,
        _: NoGcScope<'gc, '_>,
    ) -> &[PrivateField<'gc>] {
        &self.private_fields
...
impl PrivateEnvironment<'_> {
    ...
    pub(crate) fn add_static_private_field(
        ...
        println!(
            "Adding static field {:?} as {:?}",
            description.as_str(agent),
            private_name
        );
        private_name
    }
