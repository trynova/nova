...
impl GlobalEnvironmentIndex {
    ...
    pub(crate) fn has_binding(
        ...
        obj_rec.has_binding(agent, gc, scope, name)
    }
    ...
    pub(crate) fn initialize_binding(
        ...
        if dcl_rec.has_binding(agent, name) {
            ...
            obj_rec.initialize_binding(agent, gc, scope, name, value)
        }
    }
    ...
    pub(crate) fn set_mutable_binding(
        ...
        if dcl_rec.has_binding(agent, name) {
            ...
            obj_rec.set_mutable_binding(agent, gc, scope, name, value, is_strict)
        }
    }
    ...
    pub(crate) fn get_binding_value(
        ...
        if dcl_rec.has_binding(agent, n) {
            ...
            obj_rec.get_binding_value(agent, gc, scope, n, s)
        }
    }
    ...
    pub(crate) fn delete_binding(
        ...
        let existing_prop =
            has_own_property(agent, gc.reborrow(), scope.reborrow(), global_object, n)?;
        // 6. If existingProp is true, then
        if existing_prop {
            // a. Let status be ? ObjRec.DeleteBinding(N).
            let status = obj_rec.delete_binding(agent, gc, scope, name)?;
    ...
    pub(crate) fn has_restricted_global_property(
        ...
        let n = PropertyKey::from(name);
        let existing_prop = global_object.internal_get_own_property(agent, gc, scope, n)?;
    ...
    pub(crate) fn can_declare_global_var(
        ...
        if has_property {
            ...
            is_extensible(agent, gc, scope, global_object)
        }
    }
    ...
    pub(crate) fn can_declare_global_function(
        ...
        // 3. Let existingProp be ? globalObject.[[GetOwnProperty]](N).
        let existing_prop =
            global_object.internal_get_own_property(agent, gc.reborrow(), scope.reborrow(), n)?;
        // 4. If existingProp is undefined, return ? IsExtensible(globalObject).
        let Some(existing_prop) = existing_prop else {
            return is_extensible(agent, gc, scope, global_object);
        };
    ...
    pub(crate) fn create_global_var_binding(
        ...
        let extensible =
            is_extensible(agent, gc.reborrow(), scope.reborrow(), global_object).unwrap();
        // 5. If hasProperty is false and extensible is true, then
        if !has_property && extensible {
            // a. Perform ? ObjRec.CreateMutableBinding(N, D).
            obj_rec.create_mutable_binding(
                agent,
                gc.reborrow(),
                scope.reborrow(),
                name,
            ...
            obj_rec.initialize_binding(agent, gc, scope, name, Value::Undefined)?;
        }
    ...
    pub(crate) fn create_global_function_binding(
        ...
        set(agent, gc, scope, global_object, n, value, false)?;
