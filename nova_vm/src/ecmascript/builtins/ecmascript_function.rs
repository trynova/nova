...
pub(crate) struct ECMAScriptFunctionObjectHeapData {
    /// \[\[Environment]]
    pub environment: EnvironmentIndex<'static>,

    /// \[\[PrivateEnvironment]]
    pub private_environment: Option<PrivateEnvironmentIndex<'static>>,
...
pub(crate) struct OrdinaryFunctionCreateParams<'agent, 'program, 'gc> {
    ...
    pub env: EnvironmentIndex<'gc>,
    pub private_env: Option<PrivateEnvironmentIndex<'gc>>,
}
...
pub(crate) fn prepare_for_ordinary_call<'a>(
    ...
    let callee_context = ExecutionContext {
        ...
        ecmascript_code: Some(ECMAScriptCodeEvaluationState {
            lexical_environment: EnvironmentIndex::Function(local_env.unbind()),
            variable_environment: EnvironmentIndex::Function(local_env.unbind()),
