...
use crate::{
    ecmascript::{
        ...
        OrdinaryObject, ProtoIntrinsics, TemporalDuration, Value, get_difference_settings,
        get_options_object, object_handle, ordinary_populate_from_constructor,
...
pub(crate) fn to_temporal_time<'gc>(
    ...
    if let Ok(item) = Object::try_from(item) {
        ...
        if let Ok(zdt) = TemporalPlainTime::try_from(item) {
            ...
            return Ok(iso_date_time.time());
        }

        // d. Let result be ? ToTemporalTimeRecord(item).
        let result = to_temporal_record(agent, item.unbind(), gc.reborrow()).unbind()?;
