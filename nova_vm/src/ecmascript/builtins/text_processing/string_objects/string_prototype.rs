...
impl StringPrototype {
    ...
    fn split(_agent: &mut Agent, _this_value: Value, _args: ArgumentsList) -> JsResult<Value> {
        todo!()
    }
    ...
    fn to_lower_case(agent: &mut Agent, this_value: Value, _: ArgumentsList) -> JsResult<Value> {
        ...
        let lower_text = s.as_str(agent).to_lowercase();
        // 5. Let L be CodePointsToString(lowerText).
        // 6. Return L.
        Ok(String::from_str(agent, &lower_text).into_value())
    }
    ...
    fn to_upper_case(agent: &mut Agent, this_value: Value, _: ArgumentsList) -> JsResult<Value> {
        ...
        let upper_text = s.as_str(agent).to_uppercase();
        // 5. Let L be CodePointsToString(lowerText).
        // 6. Return L.
        Ok(String::from_str(agent, &upper_text).into_value())
    }
