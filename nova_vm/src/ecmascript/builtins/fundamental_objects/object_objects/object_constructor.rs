...
use crate::{
    ecmascript::{
        ...
        enumerable_own_properties, get, get_iterator, get_method, group_by_property,
        has_own_property, if_abrupt_close_iterator, iterator_close_with_error, iterator_step_value,
        ordinary_create_from_constructor, ordinary_object_create_with_intrinsics,
        require_object_coercible, same_value, set, set_integrity_level, test_integrity_level,
...
impl ObjectConstructor {
    ...
    fn define_property<'gc>(
        ...
        let mut key = if let Some(key) = to_property_key_simple(agent, p, nogc) {
            ...
            let key = to_property_key(agent, p.unbind(), gc.reborrow())
                .unbind()?
                .bind(gc.nogc());
            attributes = scoped_attributes.get(agent).bind(gc.nogc());
            key
        };
    ...
    fn freeze<'gc>(
        ...
        let status = set_integrity_level::<Frozen>(agent, o.unbind(), gc.reborrow()).unbind()?;
        if !status {
    ...
    fn seal<'gc>(
        ...
        let scoped_o = o.scope(agent, gc.nogc());
        let status = set_integrity_level::<Sealed>(agent, o.unbind(), gc.reborrow()).unbind()?;
        if !status {
            // 3. If status is false, throw a TypeError exception.
            Err(agent.throw_exception_with_static_message(
                ExceptionType::TypeError,
