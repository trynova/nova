...
pub(crate) fn get_view_byte_length(
  ...
  assert!(!is_fixed_length_array_buffer(agent, view.get_viewed_array_buffer(agent)));
...
pub(crate) fn is_view_out_of_bounds(
  ...
  if byte_offset_start > buffer_byte_length.unwrap() || byte_offset_end > buffer_byte_length.unwrap() {
    return true;
  }
