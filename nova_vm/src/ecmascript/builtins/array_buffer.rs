...
impl ArrayBuffer {
    ...
    pub(crate) fn array_buffer_byte_length(self, agent: &Agent, _order: Ordering) -> i64 {
    ...
    pub(crate) fn is_detached_buffer(self, agent: &Agent) -> bool {
    ...
    pub(crate) fn detach_array_buffer(self, agent: &mut Agent, _key: Option<DetachKey>) {
        let array_buffer = agent.heap.get_mut(self.0);
    ...
    pub(crate) fn clone_array_buffer(
    ...
    ) -> JsResult<Self> {
        {
            ...
            let src_buffer = agent.heap.get(self.0);
        ...
        let target_buffer = ArrayBuffer::allocate_array_buffer(
            agent,
    ...
    pub(crate) fn host_resize_array_buffer(self, agent: &mut Agent, new_byte_length: u64) -> bool {
        let buffer = agent.heap.get_mut(self.0);
    ...
    pub(crate) const fn is_no_tear_configuration(r#type: (), order: Ordering) -> bool {
        if Self::is_unclamped_integer_element_type(r#type) {
            ...
            true
        } else if Self::is_big_int_element_type(r#type)
