...
use crate::{
    ecmascript::{
        ...
        ProtoIntrinsics, Realm, SmallInteger, String, Value, builders::BuiltinFunctionBuilder,
        ordinary_create_from_constructor, to_number, to_primitive,
    },
...
impl DateConstructor {
    ...
    fn constructor<'gc>(
        ...
        crate::engine::bind!(let arg0 = arguments.get(0), gc);
        ...
        let dv = match number_of_args {
            ...
            1 => {
                ...
                // c. Else,
                    ...
                    let v = to_primitive(agent, value, None, gc.reborrow())?;
    ...
    fn utc<'gc>(
        ...
        let m = if arguments.len() > 1 {
            to_number(agent, month.get(agent).local(), gc.reborrow())?.to_real(agent)
        } else {
            0.0
        };
        // 3. If date is present, let dt be ? ToNumber(date); else let dt be 1𝔽.
        let dt = if arguments.len() > 2 {
            to_number(agent, date.get(agent).local(), gc.reborrow())?.to_real(agent)
        } else {
            1.0
        };
        // 4. If hours is present, let h be ? ToNumber(hours); else let h be +0𝔽.
        let h = if arguments.len() > 3 {
            to_number(agent, hours.get(agent).local(), gc.reborrow())?.to_real(agent)
        } else {
            0.0
        };
        // 5. If minutes is present, let min be ? ToNumber(minutes); else let min be +0𝔽.
        let min = if arguments.len() > 4 {
            to_number(agent, minutes.get(agent).local(), gc.reborrow())?.to_real(agent)
        } else {
            0.0
        };
        // 6. If seconds is present, let s be ? ToNumber(seconds); else let s be +0𝔽.
        let s = if arguments.len() > 5 {
            to_number(agent, seconds.get(agent).local(), gc.reborrow())?.to_real(agent)
        } else {
            0.0
        };
        // 7. If ms is present, let milli be ? ToNumber(ms); else let milli be +0𝔽.
        let milli = if arguments.len() > 6 {
            to_number(agent, ms.get(agent).local(), gc.reborrow())?.to_real(agent)
        } else {
            0.0
        };
