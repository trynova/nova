...
impl PromiseResolveThenableJob {
    pub(crate) fn run(self, agent: &mut Agent, gc: Gc<'_>, scope: Scope<'_>) -> JsResult<()> {
...
impl PromiseReactionJob {
    pub(crate) fn run(self, agent: &mut Agent, mut gc: Gc<'_>, scope: Scope<'_>) -> JsResult<()> {
        ...
        match handler_result {
            ...
            Ok(value) => {
                ...
                promise_capability.resolve(agent, gc, scope, value)
            }
        };
        Ok(())
    }
