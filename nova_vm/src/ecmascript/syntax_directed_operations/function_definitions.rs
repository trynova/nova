...
pub(crate) fn evaluate_function_body(
    ...
    Vm::execute(agent, gc, scope, exe, Some(arguments_list.0)).into_js_result()
}
...
pub(crate) fn evaluate_async_function_body(
    ...
    match Vm::execute(
        ...
        ExecutionResult::Return(result) => {
            ...
            promise_capability.resolve(agent, gc, scope, result);
        }
        ...
        ExecutionResult::Await { vm, awaited_value } => {
            ...
            let promise = Promise::resolve(agent, gc, scope, awaited_value);
