...
impl Value {
    pub fn create_exception(vm: &mut VM, message: &str) -> Value {
        let data = Gc::new(StringHeapData {
            data: Wtf8Buf::from_str(message),
        });
        vm.heap.strings.push(data);
        Value::String(vm.heap.strings.len() as u32)
    }
    ...
    pub fn is_strictly_equal(&self, vm: &VM, other: &Value) -> JsResult<bool> {
        ...
        Ok(match (self, other) {
            ...
            (Value::Number(n1), Value::Number(n2)) => {
                n1 == n2 || vm.heap.numbers[*n1 as usize].data == vm.heap.numbers[*n2 as usize].data
            }
            ...
            (Value::String(s1), Value::String(s2)) => {
                s1 == s2 || vm.heap.strings[*s1 as usize].data == vm.heap.strings[*s2 as usize].data
            }
    ...
    pub fn from_f64(vm: &mut VM, value: f64) -> Value {
        ...
        if value.is_nan() {
            ...
            vm.heap.numbers.push(Gc::new(NumberHeapData::new(value)));
    ...
    pub fn try_into_f64(&self, vm: &mut VM) -> JsResult<f64> {
        match self {
            &Value::Number(n) => Ok(vm.heap.numbers[n as usize].data),
